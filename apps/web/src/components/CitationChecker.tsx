...
export function CitationChecker({ onResults, onSelectCitation, results, formatStyle, restoredInput, onAuthOpen: _onAuthOpen }: CitationCheckerProps) {
  ...
  const hasStyledTokens = (text: string): boolean => tokenizeMarkedText(text).some((token) => token.type !== 'plain');
  ...
  const applyShortFormStyleFallback = (text: string, result: AnalyzedCitation): string => {
    if (hasStyledTokens(text)) return text;
    ...
    if (citationType === 'case') {
      ...
      const caseNameWithReporter = stripped.match(/^([^,]*\bv\.\s*[^,]+)(,.*)$/);
      if (caseNameWithReporter) {
        const [, caseName, tail] = caseNameWithReporter;
        return `*${caseName.trim()}*${tail}`;
      }
    }
    return text;
  };
